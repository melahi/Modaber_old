...
bool SolutionSimulator::isApplicable (goal *precondition, FastEnvironment *env, State &state){
	...
	if (simple){
		...
		cout << " [Literal: "; lit2->write(cout); cout << " Can't be satisfied] " << endl;
		return false;
	}
	...
	if (comp){
		...
		cout << right << " ) Can't be satisfied] " << endl;
		return false;
	}
...
void SolutionSimulator::applyAssignmentList (const pc_list <assignment *> *assignmentEffects, FastEnvironment *env, State &state){
	...
	for (; it != itEnd; ++it){
		...
		if (pne2->getStateID() != -1){
			myProblem.variables[pne2->getStateID()].domain[value];
		}
	}
}
...
//	int nValueBeforeSimulation  = countValues();
	...
	if (planIsValid && !isApplicable(current_analysis->the_problem->the_goal, &env, state)){
		planIsValid = false;
	}else if (current_analysis->the_problem->metric){
		metricValue = evaluateExpression(current_analysis->the_problem->metric->expr, &env, state);
	}
...
SolutionSimulator::SolutionSimulator():metricValue(0) {
}
